...
/**
 * Infra Action performer for AWS
 * @author Mark Kendall
 *
 */
public class AwsInfraActionPerformer extends InfraActionPerformer {
	
	private static final Logger LOG = Logger.getLogger(AwsInfraActionPerformer.class);

	private XMLGeographicContainerType environmentDefnXML;
	
	public AwsInfraActionPerformer(InfraAction action) {
		super(action);
	}
	...
	protected void doPrepare() {
		...
		LOG.info("Beginning extract of environment metadata in agnostic format from the database");
		try {
			if (environment == null) { 
	...
	protected ExecReturn doExecute() {
		...
		if (instructions == null) {
			throw new IllegalStateException("No AWS instructions in build plan for action: " + getAction().getId());
		}
